...
#include "pugixml-1.13/pugixml-1.13/src/pugixml.hpp"
...
Weather XmlService::getWeather(std::string s)
{
    ...
    double lon = node.child("city").child("coord").attribute("lon").as_float(); // 49.6601
    double lat = node.child("city").child("coord").attribute("lat").as_float(); // 58.5966
